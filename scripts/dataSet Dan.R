...
glm=glm(working ~ ., family = binomial("logit"), data = na.omit(df))
summary(glm)
glmstep=step(glm)
summary(glmstep)

glmnull=glm(working ~ 1, family = binomial("logit"), data = na.omit(df))
glmnull

cost.misc <- function(response, pred) mean(abs(response-pred) > 0.5)
1-cost.misc(df_omit$working, predict(glmstep,data=df_omit,type="response"))
1-cost.misc(df_omit$working, predict(glmnull,data=df_omit,type="response"))

predict(glmstep,data=df_omit,type="response")

max(predict(glmstep,data=df_omit,type="response"))
